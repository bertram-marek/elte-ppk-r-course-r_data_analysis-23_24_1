...
```{r}
crPlots(simple_model,terms = ~ age )

crPlots(complex_model, terms = ~  age +  STAI_trait + pain_cat + mindfulness + cortisol_serum + cortisol_saliva)
```
...
```{r}
crPlots(complex_model_updated, terms = ~  age +  STAI_trait + pain_cat + mindfulness + cortisol_serum)
```
...
```{r}
simple_model_unstandardized <- tidy(simple_model, conf.int = TRUE)

simple_model_standardized <- lm(scale(pain) ~ scale(age) + factor(sex), data = pain_data_clean)
simplem_standardized_coefficients <- tidy(simple_model_standardized)$estimate[-1]
simplem_final_table <- cbind(simple_model_unstandardized, Beta = c(NA,simplem_standardized_coefficients))

print(simplem_final_table)
simple_model_summary <- glance(simple_model)
print(as.data.frame(simple_model_summary))
```
...
```{r}
complex_model_unstandardized <- tidy(complex_model_updated, conf.int = TRUE)

complex_model_standardized <- lm(scale(pain) ~ scale(age) + factor(sex) + scale(STAI_trait) + scale(pain_cat) + scale(mindfulness) + scale(cortisol_serum), data = pain_data_clean)
complexm_standardized_coefficients <- tidy(complex_model_standardized)$estimate[-1]
complexm_final_table <- cbind(complex_model_unstandardized, Beta = c(NA,complexm_standardized_coefficients))
print(complexm_final_table)

complex_model_summary <- glance(complex_model_updated)
print(as.data.frame(complex_model_summary))
```
