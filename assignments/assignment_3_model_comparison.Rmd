...
```{r}
data_raw <- read_excel("data/assignment_3_dataset.xlsx")
```
...
report_table <- data.frame(
  Coefficient = rownames(summary$coefficients[, c("Estimate", "Std. Error")]),
  Unstandardized_Coef = coefficients[, "Estimate"],
  CI_lower = confint(model_simple)[, "2.5 %"],
  CI_upper = confint(model_simple)[, "97.5 %"],
  Standardized_Coef_B = lm.beta(model_simple)$standardized.coefficients,
  p_value = summary$coefficients[, "Pr(>|t|)"]
)
...
report_table <- data.frame(
  Coefficient = rownames(summary$coefficients[, c("Estimate", "Std. Error")]),
  Unstandardized_Coef = coefficients[, "Estimate"],
  CI_lower = confint(model_complex)[, "2.5 %"],
  CI_upper = confint(model_complex)[, "97.5 %"],
  Standardized_Coef_B = lm.beta(model_complex)$standardized.coefficients,
  p_value = summary$coefficients[, "Pr(>|t|)"]
)
