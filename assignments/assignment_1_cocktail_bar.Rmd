...
coctail_dataset$ingredients<-gsub(".*oz\\s|.*drops\\s|.*dashes\\s|\"|.*dash\\s|.*drop\\s|.*bsp\\s|\\s*\\([^\\)]+\\)|.*proof\\s|\\s+$","",as.character(coctail_dataset$ingredients))
...
```{r}
ingredients<-sort(tolower(coctail_dataset$ingredients))
ingredients_list<- ingredients[!duplicated(ingredients)]
ingredients_list
```
