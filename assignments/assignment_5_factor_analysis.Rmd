...
all.ARQ <- tidyr::gather(ARQ.data, key = "variable", value = "value", ar1:ar28)
variable_order <- paste0("ar", 1:28)
all.ARQ$variable <- factor(all.ARQ$variable, levels = variable_order)

# Relationship between party and each ARQ item
ggplot(all.ARQ, aes(x = factor(party), y = value, fill = factor(party))) +
  geom_boxplot() +
  facet_wrap(~variable, scales = "free_y", ncol = 4) +
  labs(title = "Boxplot of ARQ items by Party", x = "Party", y = "Value") +
  theme_minimal()
# Relationship between gender and each ARQ item
ggplot(all.ARQ, aes(x = factor(sex), y = value, fill = factor(sex))) +
  geom_boxplot() +
  facet_wrap(~variable, scales = "free_y", ncol = 4) +
...
```{r}
ARQ.data$sex <- factor(ARQ.data$sex, levels = c(1, 2), labels = c("male", "female"))

ARQ.data$party <- factor(ARQ.data$party, levels = c(1, 2, 3, 4), labels = c("democrat", "republican", "other", "none"))
```
...
```{r}
fa.variables <- ARQ.data[, 1:28]
```

Create the correlation matrix.

```{r}
lowerCor(fa.variables)
corr.test(fa.variables, use ="pairwise.complete.obs")$p
corr.test(fa.variables, use ="pairwise.complete.obs")$ci
```
...
```{r}
describe_result <- describe(fa.variables)
print(describe_result$skew)
print(describe_result$kurtosis)
```
...
```{r}
fa.result <- fa(fa.variables, nfactors = 5, rotate = "varimax")
scree(fa.result)
```
